...
def export_mqo(filepath, ob, rot90, invert, edge, uv_exp, uv_cor, scale):
    ...
    for mat in me.materials:
        fp.write("\t\"%s\" " % (mat.name))
        fp.write("col(%.3f %.3f %.3f %.3f)" % (mat.diffuse_color[0], mat.diffuse_color[1], mat.diffuse_color[2], mat.alpha))
        fp.write(" dif(%.3f)" % (mat.diffuse_intensity))
        fp.write(" amb(%.3f)" % (mat.ambient))
        fp.write(" emi(%.3f)" % (mat.emit))
        fp.write(" spc(%.3f)" % (mat.emit))
        fp.write(" power(5)\n")
    fp.write("}\n")
    ...
    for f in faces:
        ...
        try:
            ...
            pprint.pprint("No uv at face #%d on mesh %s" %(f.index, me.name))
